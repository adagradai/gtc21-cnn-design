...
static bool NvDsInferParseYoloV3(
        ...
        const uint &kNUM_BBOXES) {

    uint kNUM_CLASSES = detectionParams.numClassesConfigured;
    ...
    for (uint idx = 0; idx < masks.size(); ++idx) {
        ...
        std::vector<NvDsInferParseObjectInfo> outObjs = decodeYoloV3Tensor((const float *) (layer.buffer), masks[idx],
                                                                           anchors, gridSizeW, gridSizeH, stride,
                                                                           kNUM_BBOXES,
                                                                           kNUM_CLASSES, networkInfo.width,
                                                                           networkInfo.height);
...
extern "C" bool NvDsInferParseCustomPelee(
        std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
        NvDsInferNetworkInfo const &networkInfo,
        NvDsInferParseDetectionParams const &detectionParams,
        std::vector<NvDsInferParseObjectInfo> &objectList) {
    ...
    static const std::vector<float> kANCHORS = {
            11, 42, 17, 75, 23, 115, 33, 80,
            35, 155, 57, 210, 119, 347, 281, 485
    };
    ...
    const uint kNUM_BBOXES = 4;
    return NvDsInferParseYoloV3(
            outputLayersInfo, networkInfo, detectionParams, objectList,
            kANCHORS, kMASKS, kNUM_BBOXES);
}

extern "C" bool NvDsInferParseCustomHD(
        std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
        NvDsInferNetworkInfo const &networkInfo,
        NvDsInferParseDetectionParams const &detectionParams,
        std::vector<NvDsInferParseObjectInfo> &objectList) {
    ...
    static const std::vector<float> kANCHORS = {
            10.6, 21.2, 19.2, 38.1,  31.1, 59.8,  52.0, 
            94.6, 113.9, 161.0,  356.4,  331.7
    };
...
extern "C" bool NvDsInferParseCustomCD(
        std::vector<NvDsInferLayerInfo> const &outputLayersInfo,
        NvDsInferNetworkInfo const &networkInfo,
        NvDsInferParseDetectionParams const &detectionParams,
        std::vector<NvDsInferParseObjectInfo> &objectList) {
    ...
    static const std::vector<float> kANCHORS = {
            25,26,  47,58,  77,115, 105,219, 181,154, 358,328
    };
...
extern "C" bool NvDsInferParseCustomYoloTLT(
    ...
    for (int i = 0; (i < keepCount[0]) && (objectList.size() <= topK); ++i) {
        ...
        if ((loc[0] > networkInfo.width) || (loc[2] > networkInfo.width) || (loc[1] > networkInfo.height) ||
            (loc[3] > networkInfo.width))
            continue;
...
CHECK_CUSTOM_PARSE_FUNC_PROTOTYPE(NvDsInferParseCustomPelee);

CHECK_CUSTOM_PARSE_FUNC_PROTOTYPE(NvDsInferParseCustomHD);